...
def apply_art_style(image: np.ndarray, style: str) -> np.ndarray:
    ...
    if style == "pointillism":
        ...
        for _ in range(50000):
            x = random.randint(0, pil_img.width - 1)
            y = random.randint(0, pil_img.height - 1)
            color = tuple(image[y, x])
            dots.putpixel((x, y), color)
        return np.array(dots)

    elif style == "expressionist":
        img = pil_img.filter(ImageFilter.GaussianBlur(radius=2))
        img = np.array(img).astype(np.float32)
        img = np.clip(img * 1.5, 0, 255)
        return img.astype(np.uint8)

    elif style == "oil_painting":
        img = pil_img.filter(ImageFilter.ModeFilter(size=5))
        img = img.filter(ImageFilter.EDGE_ENHANCE)
        return np.array(img)

    elif style == "watercolor":
        img = pil_img.filter(ImageFilter.GaussianBlur(radius=3))
        img = np.array(img).astype(np.float32)
...
def main():
    ...
    styles = ["pointillism", "expressionist", "oil_painting", "watercolor"]
    ...
    for i, params in enumerate(param_sets_random):
        ...
        texture_styled = apply_art_style(texture, chosen_style)

        random_str = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
        filename = f"flame_texture_{random_str}_{chosen_style}.png"
        Image.fromarray(texture_styled, mode='RGB').save(filename)

        print(f"Saved {filename} | Style: {chosen_style} | Params: {params}")
