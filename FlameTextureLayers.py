...
class FlameFractal:
    ...
    def generate(self):
        ...
        for _ in range(self.config.layers):
            layer_rgb = self.generate_layer().astype(np.float32)
            combined += layer_rgb
...
def main():
    ...
    param_sets_random = [random_param_set() for _ in range(6)]

    for params in param_sets_random:
        config = Config(
            width=fixed_width,
            height=fixed_height,
            skip=fixed_skip,
        ...
        filename = f"flame_texture_multi_{random_str}.png"
